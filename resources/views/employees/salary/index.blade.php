...
<div class="content">
         ...
         <div class="col-12 table-responsive">
            <div class="card">
                <div class="pl-3 card-header" style='background-color:whitesmoke' >
                    <button type="button" name="create_record" id="create_record" class="float-right btn btn-sm btn-primary mr-3"><i class="bi bi-plus-square"></i> Add Salary</button>
					<h3 class="mb-0;ml:5"><span style="font-family: serif;color:black;weight:bolder"><strong>Salarys List</strong></span></h3>
                </div>
                ...
                <div class="card-body pt-1" >
                    ...
                    <div class='row table-responsive'>
                        <table style='width:100%'  class=" table table-bordered table-condensed mytable table-striped salary_table">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Employee</th>
    ...
    <div class="modal fade" id="formModal" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
        ...
        <form method="post" id="sample_form" class="form-horizontal">
            <div class="modal-header" style="background-color: lightgray; text-align: center;">
                <h5 class="modal-title" id="ModalLabel" style="color: white;">Add New Salary</h5>
            ...
            <div class="modal-body" style="max-height: 400px; overflow-y: auto;">
                ...
                <div class="form-group">
                    ...
                    <select name="employee_id" id="employee_id" class="form-control">
                        ...
                        @foreach (App\Models\user_management\Employee::orderBy('name')->where('is_deleted', '0')->get() as $key => $value)
                ...
                <div class="form-group">
                    <label>Salary Per Month : <span style="color: red;">&#10033;</span></label>
                    <input type="text" name="salary_per_month" id="salary_per_month" class="form-control" />
                </div>
                <div class="form-group">
                    <label>Hired Date : <span style="color: red;">&#10033;</span></label>
    ...
    <div class="modal fade" id="attributeModal" tabindex="-1" role="dialog" aria-labelledby="attributeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header" style="background-color: lightgray; text-align: center;">
                    <h5 class="modal-title" id="attributeModalLabel" style="color: white;">Salary Details</h5>
...
$(document).ready(function() {
    var table = $('.salary_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('salary.view_salary') }}",
        
        columns: [
            {data: 'number', name: 'number'},
            {data: 'employee_id', name: 'employee_id'},
            {data: 'salary_per_month', name: 'salary_per_month'},
    ...
    dataTableSearch = $('.salary_table').DataTable();  
    ...
    $('#sample_form').on('submit', function(event){
        ...
        if($('#action').val() == 'Add')
        {
            action_url = "{{ route('salary.store_salary') }}";
        }
 
        if($('#action').val() == 'Edit')
        {
            action_url = "{{ route('salary.update_salary') }}";
        }
    ...
    $(document).on('click', '.edit', function(event){
        ...
        $.ajax({
            ...
            success:function(data)
            {
                ...
                $('#salary_per_month').val(data.result.salary_per_month);
                $('#hired_date').val(data.result.hired_date);
                $('#hidden_id').val(id);
    ...
    $(document).on('click', '.detail', function() {
        ...
        $.ajax({
            url :"/employees/salary/show/"+id+"/",
            type: "GET",
            dataType: "json",
            success: function(response) {
                var attribute = response.attribute;
                var employename = response.employename;

                // Populate the modal with attribute data
                $('#attributeDetails0').html('<p>'+'<strong> Employee Name:</strong>'+'<br>' + employename + '</p>');
                $('#attributeDetails1').html('<p>'+'<strong> Salary Per Month:</strong>'+'<br>' + attribute.salary_per_month + '</p>');
                $('#attributeDetails2').html('<p>'+'<strong> Total Salary:</strong>'+'<br>' + attribute.salary_total + '</p>');
                $('#attributeDetails3').html('<p>'+'<strong> Left Salary:</strong>'+'<br>' + attribute.salary_left + '</p>');
                $('#attributeDetails4').html('<p>'+'<strong> Hired Date:</strong>'+'<br>' + attribute.hired_date + '</p>');
    ...
    $(document).on('click', '.delete', function(){
        salary_id = $(this).attr('id');
        $('.modal-title').text('Delete Salary');
        $('#confirmModal').modal('show');
    });
 
    $('#ok_button').click(function(){
        $.ajax({
            url:"/employees/salary/delete/"+salary_id,
            beforeSend:function(){
