...
@section('title', 'Employee Credit | Dashboard')
        ...
        <div class="breadcrumb-line breadcrumb-line-light header-elements-lg-inline">
            <div class="d-flex">
                <div class="breadcrumb">
                    ...
                    <span class="breadcrumb-item active">Employee Credit</span>
                </div>
...
<div class="content">
         ...
         <div class="col-12 table-responsive">
            <div class="card">
                <div class="pl-3 card-header" style='background-color:whitesmoke' >
                    <button type="button" name="create_record" id="create_record" class="float-right btn btn-sm btn-primary mr-3"><i class="bi bi-plus-square"></i> Add Credit</button>
					<h3 class="mb-0;ml:5"><span style="font-family: serif;color:black;weight:bolder"><strong>Credits List</strong></span></h3>
                </div>
                ...
                <div class="card-body pt-1" >
                    ...
                    <div class='row table-responsive'>
                        <table style='width:100%'  class=" table table-bordered table-condensed mytable table-striped credit_table">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Employee</th>
    ...
    <div class="modal fade" id="formModal" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
        ...
        <form method="post" id="sample_form" class="form-horizontal">
            <div class="modal-header" style="background-color: lightgray; text-align: center;">
                <h5 class="modal-title" id="ModalLabel" style="color: white;">Add New Credit</h5>
            ...
            <div class="modal-body" style="max-height: 400px; overflow-y: auto;">
                ...
                <div class="form-group">
                    ...
                    <select name="salary_id" id="salary_id" class="form-control">
                        ...
                        @foreach (App\Models\user_management\Salary::orderBy('id')->where('is_deleted', '0')->get() as $key => $value)
    ...
    <div class="modal fade" id="attributeModal" tabindex="-1" role="dialog" aria-labelledby="attributeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header" style="background-color: lightgray; text-align: center;">
                    <h5 class="modal-title" id="attributeModalLabel" style="color: white;">Credit Details</h5>
...
$(document).ready(function() {
    ...
    dataTableSearch = $('.credit_table').DataTable();  
    ...
    $('#create_record').click(function(){
        $('.modal-title').text('Add New Credit');
    ...
    $('#sample_form').on('submit', function(event){
        ...
        if($('#action').val() == 'Add')
        {
            action_url = "{{ route('credit.store_credit') }}";
        }
 
        if($('#action').val() == 'Edit')
        {
            action_url = "{{ route('credit.update_credit') }}";
        }
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if(data.success)
                {
                    ...
                    $('#sample_form')[0].reset();
                    $('#credit_table').DataTable().ajax.reload();
    ...
    $(document).on('click', '.edit', function(event){
        ...
        $.ajax({
            url :"/employees/credit/edit/"+id+"/",
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            dataType:"json",
            success:function(data)
            {
                console.log('success: '+data);
                $('#salary_id').val(data.result.salary_id);
                $('#credit').val(data.result.credit);
                $('#reason').val(data.result.reason);
                $('#date').val(data.result.date);
                $('#hidden_id').val(id);
                $('.modal-title').text('Edit Credit');
    ...
    $(document).on('click', '.detail', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#attributeDetails1').html('<p>'+'<strong> Credit:</strong>'+'<br>' + attribute.credit + '</p>');
                $('#attributeDetails2').html('<p>'+'<strong> Reason:</strong>'+'<br>' + attribute.reason + '</p>');
    ...
    $(document).on('click', '.delete', function(){
        credit_id = $(this).attr('id');
        $('.modal-title').text('Delete Credit');
        $('#confirmModal').modal('show');
    });
 
    $('#ok_button').click(function(){
        $.ajax({
            url:"/employees/credit/delete/"+credit_id,
            beforeSend:function(){
