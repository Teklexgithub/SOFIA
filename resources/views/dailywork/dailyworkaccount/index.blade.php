...
@section('title', 'Account Hisabi | Dashboard')
        ...
        <div class="breadcrumb-line breadcrumb-line-light header-elements-lg-inline">
            <div class="d-flex">
                <div class="breadcrumb">
                    ...
                    <span class="breadcrumb-item active">Account Hisabi</span>
                </div>
...
<div class="content">
         ...
         <div class="col-12 table-responsive">
            <div class="card">
                <div class="pl-3 card-header" style='background-color:whitesmoke' >
                    <button type="button" name="create_record" id="create_record" class="float-right btn btn-sm btn-primary mr-3"><i class="bi bi-plus-square"></i> Add Account Hisabi</button>
					<h3 class="mb-0;ml:5"><span style="font-family: serif;color:black;weight:bolder"><strong>በአካውንት የገባ</strong></span></h3>
                </div>
                ...
                <div class="card-body pt-1" >
                    ...
                    <div class='row table-responsive'>
                        <table style='width:100%'  class=" table table-bordered table-condensed mytable table-striped dailyworkaccount_table">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Branch</th>
    ...
    <div class="modal fade" id="formModal" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
        ...
        <form method="post" id="sample_form" class="form-horizontal">
            <div class="modal-header" style="background-color: lightgray; text-align: center;">
                <h5 class="modal-title" id="ModalLabel" style="color: white;">Add New Account Hisabi</h5>
            ...
            <div class="modal-body" style="max-height: 400px; overflow-y: auto;">
                ...
                @endrole
                <div class="form-group">
                <label>Bank/Telebirr : <span style="color: red;">&#10033;</span></label>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="birr_amount" id="birr_amount" class="form-control" />
                </div>
                <div class="form-group">
    ...
    <div class="modal fade" id="attributeModal" tabindex="-1" role="dialog" aria-labelledby="attributeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header" style="background-color: lightgray; text-align: center;">
                    <h5 class="modal-title" id="attributeModalLabel" style="color: white;">Account Hisabi Details</h5>
...
$(document).ready(function() {
    var table = $('.dailyworkaccount_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('dailyworkaccount.view_dailyworkaccount') }}",
    ...
    dataTableSearch = $('.dailyworkaccount_table').DataTable();  
    ...
    $('#create_record').click(function(){
        $('.modal-title').text('Add New Account Hisabi');
    ...
    $('#sample_form').on('submit', function(event){
        ...
        if($('#action').val() == 'Add')
        {
            action_url = "{{ route('dailyworkaccount.store_dailyworkaccount') }}";
        }
 
        if($('#action').val() == 'Edit')
        {
            action_url = "{{ route('dailyworkaccount.update_dailyworkaccount') }}";
        }
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if(data.success)
                {
                    ...
                    $('#sample_form')[0].reset();
                    $('#dailyworkaccount_table').DataTable().ajax.reload();
    ...
    $(document).on('click', '.edit', function(event){
        ...
        $.ajax({
            url :"/dailywork/dailyworkaccount/edit/"+id+"/",
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            dataType:"json",
            success:function(data)
            {
                ...
                $('#birr_amount').val(data.result.birr_amount);
                $('#date').val(data.result.date);
                $('#hidden_id').val(id);
                $('.modal-title').text('Edit Account Hisabi');
    ...
    $(document).on('click', '.detail', function() {
        ...
        $.ajax({
            url :"/dailywork/dailyworkaccount/show/"+id+"/",
            type: "GET",
            dataType: "json",
            success: function(response) {
                ...
                $('#attributeDetails2').html('<p>'+'<strong> Birr:</strong>'+'<br>' + attribute.birr_amount + '</p>');        
                $('#attributeDetails3').html('<p>'+'<strong> Type:</strong>'+'<br>' + attribute.type + '</p>');
    ...
    var dailyworkaccount_id;
 
    $(document).on('click', '.delete', function(){
        dailyworkaccount_id = $(this).attr('id');
        $('.modal-title').text('Delete Account Hisabi');
        $('#confirmModal').modal('show');
    });
 
    $('#ok_button').click(function(){
        $.ajax({
            url:"/dailywork/dailyworkaccount/delete/"+dailyworkaccount_id,
            beforeSend:function(){
