...
@section('title', 'Woci Daily | Dashboard')
        ...
        <div class="breadcrumb-line breadcrumb-line-light header-elements-lg-inline">
            <div class="d-flex">
                <div class="breadcrumb">
                    ...
                    <span class="breadcrumb-item active">Woci Daily</span>
                </div>
...
<div class="content">
         ...
         <div class="col-12 table-responsive">
            <div class="card">
                <div class="pl-3 card-header" style='background-color:whitesmoke' >
                    <button type="button" name="create_record" id="create_record" class="float-right btn btn-sm btn-primary mr-3"><i class="bi bi-plus-square"></i> Add Woci Daily</button>
					<h3 class="mb-0;ml:5"><span style="font-family: serif;color:black;weight:bolder"><strong>የወጣ ወጭ</strong></span></h3>
                </div>
                ...
                <div class="card-body pt-1" >
                    ...
                    <div class='row table-responsive'>
                        <table style='width:100%'  class=" table table-bordered table-condensed mytable table-striped dailyworkwoci_table">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Branch</th>
    ...
    <div class="modal fade" id="formModal" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
        ...
        <form method="post" id="sample_form" class="form-horizontal">
            <div class="modal-header" style="background-color: lightgray; text-align: center;">
                <h5 class="modal-title" id="ModalLabel" style="color: white;">Add New Woci Daily</h5>
    ...
    <div class="modal fade" id="attributeModal" tabindex="-1" role="dialog" aria-labelledby="attributeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header" style="background-color: lightgray; text-align: center;">
                    <h5 class="modal-title" id="attributeModalLabel" style="color: white;">Birr Woci Details</h5>
...
$(document).ready(function() {
    var table = $('.dailyworkwoci_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('dailyworkwoci.view_dailyworkwoci') }}",
        
        columns: [
            {data: 'number', name: 'number'},
            {data: 'branch_id', name: 'branch_id'},
            {data: 'birr_amount', name: 'birr_amount'},
    ...
    dataTableSearch = $('.dailyworkwoci_table').DataTable();  
    ...
    $('#create_record').click(function(){
        $('.modal-title').text('Add New Woci Daily');
    ...
    $('#sample_form').on('submit', function(event){
        ...
        if($('#action').val() == 'Add')
        {
            action_url = "{{ route('dailyworkwoci.store_dailyworkwoci') }}";
        }
 
        if($('#action').val() == 'Edit')
        {
            action_url = "{{ route('dailyworkwoci.update_dailyworkwoci') }}";
        }
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if(data.success)
                {
                    ...
                    $('#sample_form')[0].reset();
                    $('#dailyworkwoci_table').DataTable().ajax.reload();
    ...
    $(document).on('click', '.edit', function(event){
        ...
        $.ajax({
            url :"/dailywork/dailyworkwoci/edit/"+id+"/",
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            dataType:"json",
            success:function(data)
            {
                ...
                $('#hidden_id').val(id);
                $('.modal-title').text('Edit Woci Daily');
    ...
    $(document).on('click', '.detail', function() {
        ...
        $.ajax({
            url :"/dailywork/dailyworkwoci/show/"+id+"/",
            type: "GET",
            dataType: "json",
            success: function(response) {
                var attribute = response.attribute;
                var branchname = response.branchname;
                var birr_amount = response.birr_amount;
                ...
                $('#attributeDetails1').html('<p>'+'<strong> Reason:</strong>'+'<br>' + attribute.name + '</p>');
                $('#attributeDetails2').html('<p>'+'<strong> Birr:</strong>'+'<br>' + birr_amount + '</p>');
    ...
    var dailyworkwoci_id;
 
    $(document).on('click', '.delete', function(){
        dailyworkwoci_id = $(this).attr('id');
        $('.modal-title').text('Delete Woci Daily');
        $('#confirmModal').modal('show');
    });
 
    $('#ok_button').click(function(){
        $.ajax({
            url:"/dailywork/dailyworkwoci/delete/"+dailyworkwoci_id,
            beforeSend:function(){
