...
@section('title', 'Yetekefele Hisabi | Dashboard')
        ...
        <div class="breadcrumb-line breadcrumb-line-light header-elements-lg-inline">
            <div class="d-flex">
                <div class="breadcrumb">
                    ...
                    <span class="breadcrumb-item active">Yetekefele Hisabi</span>
                </div>
...
<div class="content">
         ...
         <div class="col-12 table-responsive">
            <div class="card">
                <div class="pl-3 card-header" style='background-color:whitesmoke' >
                    <button type="button" name="create_record" id="create_record" class="float-right btn btn-sm btn-primary mr-3"><i class="bi bi-plus-square"></i> Add Yetekefele Hisabi</button>
					<h3 class="mb-0;ml:5"><span style="font-family: serif;color:black;weight:bolder"><strong>የተከፈለ ዱቤ</strong></span></h3>
                </div>
                ...
                <div class="card-body pt-1" >
                    ...
                    <div class='row table-responsive'>
                        <table style='width:100%'  class=" table table-bordered table-condensed mytable table-striped dailyworkyetekefele_table">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Branch</th>
    ...
    <div class="modal fade" id="formModal" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
        ...
        <form method="post" id="sample_form" class="form-horizontal">
            <div class="modal-header" style="background-color: lightgray; text-align: center;">
                <h5 class="modal-title" id="ModalLabel" style="color: white;">Add New Yetekefele Hisabi</h5>
            ...
            <div class="modal-body" style="max-height: 400px; overflow-y: auto;">
                ...
                <select name="credit_id" id="credit_id" class="form-control">
                    ...
                    @php
                        $query = App\Models\daily_work_management\Dailyworkcredit::orderBy('name')
                                    ->where('is_deleted', '0')
                                    ->where('is_paid', '0');
                        if ($branch_id) {
                ...
                <div class="form-group">
                    <label>Birr : <span style="color: red;">&#10033;</span></label>
    ...
    <div class="modal fade" id="attributeModal" tabindex="-1" role="dialog" aria-labelledby="attributeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header" style="background-color: lightgray; text-align: center;">
                    <h5 class="modal-title" id="attributeModalLabel" style="color: white;">Yetekefele Hisabi Details</h5>
...
$(document).ready(function() {
    var table = $('.dailyworkyetekefele_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('dailyworkyetekefele.view_dailyworkyetekefele') }}",
    ...
    dataTableSearch = $('.dailyworkyetekefele_table').DataTable();  
    ...
    $('#create_record').click(function(){
        $('.modal-title').text('Add New Yetekefele Hisabi');
    ...
    $('#sample_form').on('submit', function(event){
        ...
        if($('#action').val() == 'Add')
        {
            action_url = "{{ route('dailyworkyetekefele.store_dailyworkyetekefele') }}";
        }
 
        if($('#action').val() == 'Edit')
        {
            action_url = "{{ route('dailyworkyetekefele.update_dailyworkyetekefele') }}";
        }
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if(data.success)
                {
                    ...
                    $('#sample_form')[0].reset();
                    $('#dailyworkyetekefele_table').DataTable().ajax.reload();
    ...
    $(document).on('click', '.edit', function(event){
        ...
        $.ajax({
            url :"/dailywork/dailyworkyetekefele/edit/"+id+"/",
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            dataType:"json",
            success:function(data)
            {
                ...
                $('#credit_id').val(data.result.credit_id);
                $('#paid_amount').val(data.result.paid_amount);
                $('#date').val(data.result.date);
                $('#hidden_id').val(id);
                $('.modal-title').text('Edit Yetekefele Hisabi');
    ...
    $(document).on('click', '.detail', function() {
        ...
        $.ajax({
            url :"/dailywork/dailyworkyetekefele/show/"+id+"/",
            type: "GET",
            dataType: "json",
            success: function(response) {
                var attribute = response.attribute;
                var branchname = response.branchname;
                var creditname = response.creditname;
                
                // Populate the modal with attribute data
                $('#attributeDetails0').html('<p>'+'<strong> Branch:</strong>'+'<br>' + branchname + '</p>');
                $('#attributeDetails1').html('<p>'+'<strong> Name:</strong>'+'<br>' + creditname + '</p>');
                $('#attributeDetails2').html('<p>'+'<strong> Birr:</strong>'+'<br>' + attribute.paid_amount + '</p>');        
                $('#attributeDetails3').html('<p>'+'<strong> Date:</strong>'+'<br>' + attribute.date + '</p>');
    ...
    var dailyworkyetekefele_id;
 
    $(document).on('click', '.delete', function(){
        dailyworkyetekefele_id = $(this).attr('id');
        $('.modal-title').text('Delete Yetekefele Hisabi');
        $('#confirmModal').modal('show');
    });
 
    $('#ok_button').click(function(){
        $.ajax({
            url:"/dailywork/dailyworkyetekefele/delete/"+dailyworkyetekefele_id,
            beforeSend:function(){
