...
@section('title', 'Yemeta khat | Dashboard')
        ...
        <div class="breadcrumb-line breadcrumb-line-light header-elements-lg-inline">
            <div class="d-flex">
                <div class="breadcrumb">
                    ...
                    <span class="breadcrumb-item active">Yemeta Khat</span>
                </div>
...
<div class="content">
         ...
         <div class="col-12 table-responsive">
            <div class="card">
                <div class="pl-3 card-header" style='background-color:whitesmoke' >
                    <button type="button" name="create_record" id="create_record" class="float-right btn btn-sm btn-primary mr-3"><i class="bi bi-plus-square"></i> Add Yemeta Khat</button>
					<h3 class="mb-0;ml:5"><span style="font-family: serif;color:black;weight:bolder"><strong>የመጣ ጫት</strong></span></h3>
                </div>
                ...
                <div class="card-body pt-1" >
                    ...
                    <div class='row table-responsive'>
                        <table style='width:100%'  class=" table table-bordered table-condensed mytable table-striped yemetakhat_table">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Branch</th>
    ...
    <div class="modal fade" id="formModal" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
        ...
        <form method="post" id="sample_form" class="form-horizontal">
            <div class="modal-header" style="background-color: lightgray; text-align: center;">
                <h5 class="modal-title" id="ModalLabel" style="color: white;">Add New Yemeta khat</h5>
            ...
            <div class="modal-body" style="max-height: 400px; overflow-y: auto;">
                ...
                <div class="form-group">
                    <label>Khat : <span style="color: red;">&#10033;</span></label>
                    <select name="khat_id" id="khat_id" class="form-control">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="yemeta_khat" id="yemeta_khat" class="form-control" />
                </div>
                <div class="form-group">
    ...
    <div class="modal fade" id="attributeModal" tabindex="-1" role="dialog" aria-labelledby="attributeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header" style="background-color: lightgray; text-align: center;">
                    <h5 class="modal-title" id="attributeModalLabel" style="color: white;">Yemeta khat Details</h5>
...
$(document).ready(function() {
    var table = $('.yemetakhat_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('yemetakhat.view_yemetakhat') }}",
        
        columns: [
            {data: 'number', name: 'number'},
            {data: 'branch_id', name: 'branch_id'},
            {data: 'khat_id', name: 'khat_id'},
            {data: 'yemeta_khat', name: 'yemeta_khat'},
    ...
    dataTableSearch = $('.yemetakhat_table').DataTable();  
    ...
    $('#create_record').click(function(){
        $('.modal-title').text('Add New Yemeta Khat');
    ...
    $('#sample_form').on('submit', function(event){
        ...
        if($('#action').val() == 'Add')
        {
            action_url = "{{ route('yemetakhat.store_yemetakhat') }}";
        }
 
        if($('#action').val() == 'Edit')
        {
            action_url = "{{ route('yemetakhat.update_yemetakhat') }}";
        }
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if(data.success)
                {
                    ...
                    $('#sample_form')[0].reset();
                    $('#yemetakhat_table').DataTable().ajax.reload();
    ...
    $(document).on('click', '.edit', function(event){
        ...
        $.ajax({
            url :"/dailywork/yemetakhat/edit/"+id+"/",
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            dataType:"json",
            success:function(data)
            {
                ...
                $('#khat_id').val(data.result.khat_id);
                $('#yemeta_khat').val(data.result.yemeta_khat);
                $('#date').val(data.result.date);
                $('#hidden_id').val(id);
                $('.modal-title').text('Edit Yemeta Khat');
    ...
    $(document).on('click', '.detail', function() {
        ...
        $.ajax({
            url :"/dailywork/yemetakhat/show/"+id+"/",
            type: "GET",
            dataType: "json",
            success: function(response) {
                var attribute = response.attribute;
                var branchname = response.branchname;
                var khatname = response.khatname;

                // Populate the modal with attribute data
                $('#attributeDetails0').html('<p>'+'<strong> Branch:</strong>'+'<br>' + branchname + '</p>');
                $('#attributeDetails1').html('<p>'+'<strong> Khat:</strong>'+'<br>' + khatname + '</p>');
                $('#attributeDetails2').html('<p>'+'<strong> Amount:</strong>'+'<br>' + attribute.yemeta_khat + '</p>');
    ...
    $(document).on('click', '.delete', function(){
        yemetakhat_id = $(this).attr('id');
        $('.modal-title').text('Delete Yemeta Khat');
        $('#confirmModal').modal('show');
    });
 
    $('#ok_button').click(function(){
        $.ajax({
            url:"/dailywork/yemetakhat/delete/"+yemetakhat_id,
            beforeSend:function(){
