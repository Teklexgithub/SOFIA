...
@section('title', 'Gudilet Daily | Dashboard')
        ...
        <div class="breadcrumb-line breadcrumb-line-light header-elements-lg-inline">
            <div class="d-flex">
                <div class="breadcrumb">
                    ...
                    <span class="breadcrumb-item active">Gudilet Daily</span>
                </div>
         ...
         <div class="col-12 table-responsive">
            <div class="card">
                <div class="pl-3 card-header" style='background-color:whitesmoke' >
                    <button type="button" name="create_record" id="create_record" class="float-right btn btn-sm btn-primary mr-3"><i class="bi bi-plus-square"></i> Add Gudilet Daily</button>
					<h3 class="mb-0;ml:5"><span style="font-family: serif;color:black;weight:bolder"><strong>የጎደለ ሕሳብ</strong></span></h3>
                </div>
                ...
                <div class="card-body pt-1" >
                    ...
                    <div class='row table-responsive'>
                        <table style='width:100%'  class=" table table-bordered table-condensed mytable table-striped dailyworkgudilet_table">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Branch</th>
    ...
    <div class="modal fade" id="formModal" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
        ...
        <form method="post" id="sample_form" class="form-horizontal">
            <div class="modal-header" style="background-color: lightgray; text-align: center;">
                <h5 class="modal-title" id="ModalLabel" style="color: white;">Add New Gudilet Daily</h5>
            ...
            <div class="modal-body" style="max-height: 400px; overflow-y: auto;">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="gudilet" id="gudilet" class="form-control" />
                </div>
                <div class="form-group">
...
$(document).ready(function() {
    var table = $('.dailyworkgudilet_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('dailyworkgudilet.view_dailyworkgudilet') }}",
        
        columns: [
            {data: 'number', name: 'number'},
            {data: 'branch_id', name: 'branch_id'},
            {data: 'gudilet', name: 'gudilet'},
            {data: 'date', name: 'date'},
    ...
    dataTableSearch = $('.dailyworkgudilet_table').DataTable();  
    ...
    $('#create_record').click(function(){
        $('.modal-title').text('Add New Gudilet Daily');
    ...
    $('#sample_form').on('submit', function(event){
        ...
        if($('#action').val() == 'Add')
        {
            action_url = "{{ route('dailyworkgudilet.store_dailyworkgudilet') }}";
        }
 
        if($('#action').val() == 'Edit')
        {
            action_url = "{{ route('dailyworkgudilet.update_dailyworkgudilet') }}";
        }
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if(data.success)
                {
                    ...
                    $('#sample_form')[0].reset();
                    $('#dailyworkgudilet_table').DataTable().ajax.reload();
    ...
    $(document).on('click', '.edit', function(event){
        ...
        $.ajax({
            url :"/dailywork/dailyworkgudilet/edit/"+id+"/",
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            dataType:"json",
            success:function(data)
            {
                ...
                $('#gudilet').val(data.result.gudilet);
                $('#date').val(data.result.date);
                $('#hidden_id').val(id);
                $('.modal-title').text('Edit Gudilet Daily');
    ...
    $(document).on('click', '.detail', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#attributeDetails1').html('<p>'+'<strong> Birr:</strong>'+'<br>' + gudilet + '</p>');
                $('#attributeDetails2').html('<p>'+'<strong> Date:</strong>'+'<br>' + attribute.date + '</p>');         
    ...
    var dailyworkgudilet_id;
 
    $(document).on('click', '.delete', function(){
        dailyworkgudilet_id = $(this).attr('id');
        $('.modal-title').text('Delete Gudilet Daily');
        $('#confirmModal').modal('show');
    });
 
    $('#ok_button').click(function(){
        $.ajax({
            url:"/dailywork/dailyworkgudilet/delete/"+dailyworkgudilet_id,
            beforeSend:function(){
