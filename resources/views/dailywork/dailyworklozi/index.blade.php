...
@section('title', 'Lozi Hisabi | Dashboard')
        ...
        <div class="breadcrumb-line breadcrumb-line-light header-elements-lg-inline">
            <div class="d-flex">
                <div class="breadcrumb">
                    ...
                    <span class="breadcrumb-item active">Lozi Hisabi</span>
                </div>
...
<div class="content">
         ...
         <div class="col-12 table-responsive">
            <div class="card">
                <div class="pl-3 card-header" style='background-color:whitesmoke' >
                    <button type="button" name="create_record" id="create_record" class="float-right btn btn-sm btn-primary mr-3"><i class="bi bi-plus-square"></i> Add Lozi Hisabi</button>
					<h3 class="mb-0;ml:5"><span style="font-family: serif;color:black;weight:bolder"><strong>የሎዝ ሕሳብ</strong></span></h3>
                </div>
                ...
                <div class="card-body pt-1" >
                    ...
                    <div class='row table-responsive'>
                        <table style='width:100%'  class=" table table-bordered table-condensed mytable table-striped dailyworklozi_table">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Branch</th>
    ...
    <div class="modal fade" id="formModal" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
        ...
        <form method="post" id="sample_form" class="form-horizontal">
            <div class="modal-header" style="background-color: lightgray; text-align: center;">
                <h5 class="modal-title" id="ModalLabel" style="color: white;">Add New Lozi Hisabi</h5>
            ...
            <div class="modal-body" style="max-height: 400px; overflow-y: auto;">
                ...
                <div class="form-group">
                <label>Lozi : <span style="color: red;">&#10033;</span></label>
                <select name="lozi_id" id="lozi_id" class="form-control">
                    <option value=" ">-- Select Lozi --</option>
                    @foreach (App\Models\material_management\Lozi::orderBy('name')->where('status','1')->get() as $key => $value)   
                        <option value="{{$value->id}}">
                            {{$value->name}}
                        </option>  
                    @endforeach
                ...
                <div class="form-group">
                    <label>Yemeta : <span style="color: red;">&#10033;</span></label>
                    <input type="text" name="yemeta" id="yemeta" class="form-control" />
                </div>              
                <div class="form-group">
                    <label>Yalew Lozi : <span style="color: red;">&#10033;</span></label>
                    <input type="text" name="yadere_lozi" id="yadere_lozi" class="form-control" />
                </div>
                <div class="form-group">
    ...
    <div class="modal fade" id="attributeModal" tabindex="-1" role="dialog" aria-labelledby="attributeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header" style="background-color: lightgray; text-align: center;">
                    <h5 class="modal-title" id="attributeModalLabel" style="color: white;">Lozi Hisabi Details</h5>
...
$(document).ready(function() {
    var table = $('.dailyworklozi_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('dailyworklozi.view_dailyworklozi') }}",
        
        columns: [
            {data: 'number', name: 'number'},
            {data: 'branch_id', name: 'branch_id'},
            {data: 'lozi_id', name: 'lozi_id'},
            {data: 'total_lozi', name: 'total_lozi'},
    ...
    dataTableSearch = $('.dailyworklozi_table').DataTable();  
    ...
    $('#create_record').click(function(){
        $('.modal-title').text('Add New Lozi Hisabi');
    ...
    $('#sample_form').on('submit', function(event){
        ...
        if($('#action').val() == 'Add')
        {
            action_url = "{{ route('dailyworklozi.store_dailyworklozi') }}";
        }
 
        if($('#action').val() == 'Edit')
        {
            action_url = "{{ route('dailyworklozi.update_dailyworklozi') }}";
        }
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if(data.success)
                {
                    ...
                    $('#sample_form')[0].reset();
                    $('#dailyworklozi_table').DataTable().ajax.reload();
    ...
    $(document).on('click', '.edit', function(event){
        ...
        $.ajax({
            url :"/dailywork/dailyworklozi/edit/"+id+"/",
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            dataType:"json",
            success:function(data)
            {
                ...
                $('#lozi_id').val(data.result.lozi_id);
                $('#yemeta').val(data.result.yemeta);
                $('#yadere_lozi').val(data.result.yadere_lozi);
                $('#date').val(data.result.date);
                $('#hidden_id').val(id);
                $('.modal-title').text('Edit Lozi Hisabi');
    ...
    $(document).on('click', '.detail', function() {
        ...
        $.ajax({
            url :"/dailywork/dailyworklozi/show/"+id+"/",
            type: "GET",
            dataType: "json",
            success: function(response) {
                var attribute = response.attribute;
                var branchname = response.branchname;
                var loziname = response.loziname;
                
                
                // Populate the modal with attribute data
                $('#attributeDetails0').html('<p>'+'<strong> Branch:</strong>'+'<br>' + branchname + '</p>');
                $('#attributeDetails1').html('<p>'+'<strong> Lozi:</strong>'+'<br>' + loziname + '</p>');
                $('#attributeDetails2').html('<p>'+'<strong> Yemeta:</strong>'+'<br>' + attribute.yemeta + '</p>');
                $('#attributeDetails3').html('<p>'+'<strong> Total Lozi:</strong>'+'<br>' + attribute.total_lozi + '</p>');        
                $('#attributeDetails4').html('<p>'+'<strong> Yalew Lozi:</strong>'+'<br>' + attribute.yadere_lozi + '</p>');
                $('#attributeDetails5').html('<p>'+'<strong> Yeteshete Lozi:</strong>'+'<br>' + attribute.yeteshete_lozi + '</p>');
    ...
    var dailyworklozi_id;
 
    $(document).on('click', '.delete', function(){
        dailyworklozi_id = $(this).attr('id');
        $('.modal-title').text('Delete Lozi Hisabi');
        $('#confirmModal').modal('show');
    });
 
    $('#ok_button').click(function(){
        $.ajax({
            url:"/dailywork/dailyworklozi/delete/"+dailyworklozi_id,
            beforeSend:function(){
