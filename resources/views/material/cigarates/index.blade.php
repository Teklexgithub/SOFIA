...
@section('title', 'Cigarates | Dashboard')
        ...
        <div class="breadcrumb-line breadcrumb-line-light header-elements-lg-inline">
            <div class="d-flex">
                <div class="breadcrumb">
                    ...
                    <span class="breadcrumb-item active">Cigarates</span>
                </div>
...
<div class="content">
         ...
         <div class="col-12 table-responsive">
            <div class="card">
                <div class="pl-3 card-header" style='background-color:whitesmoke' >
                    <button type="button" name="create_record" id="create_record" class="float-right btn btn-sm btn-primary mr-3"><i class="bi bi-plus-square"></i> Add Cigarate</button>
					<h3 class="mb-0;ml:5"><span style="font-family: serif;color:black;weight:bolder"><strong>Cigarates List</strong></span></h3>
                </div>
                ...
                <div class="card-body pt-1" >
                    ...
                    <div class='row table-responsive'>
                        <table style='width:100%'  class=" table table-bordered table-condensed mytable table-striped cigarate_table">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Name</th>
    ...
    <div class="modal fade" id="formModal" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
        ...
        <form method="post" id="sample_form" class="form-horizontal">
            <div class="modal-header" style="background-color: lightgray; text-align: center;">
                <h5 class="modal-title" id="ModalLabel" style="color: white;">Add New Cigarate</h5>
            ...
            <div class="modal-body" style="max-height: 400px; overflow-y: auto;">
                ...
                <div class="form-group">
                    <label>Packet-Price : <span style="color: red;">&#10033;</span></label>
                    <input type="text" name="packet_price" id="packet_price" class="form-control" />
                </div>
                <div class="form-group">
    ...
    <div class="modal fade" id="attributeModal" tabindex="-1" role="dialog" aria-labelledby="attributeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header" style="background-color: lightgray; text-align: center;">
                    <h5 class="modal-title" id="attributeModalLabel" style="color: white;">Cigarates Details</h5>
...
$(document).ready(function() {
    var table = $('.cigarate_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('cigarate.view_cigarate') }}",
        
        columns: [
            {data: 'number', name: 'number'},
            {data: 'name', name: 'name'},
            {data: 'packet_price', name: 'packet_price'},
    ...
    dataTableSearch = $('.cigarate_table').DataTable();  
    ...
    $('#create_record').click(function(){
        $('.modal-title').text('Add New Cigarate');
    ...
    $('#sample_form').on('submit', function(event){
        ...
        if($('#action').val() == 'Add')
        {
            action_url = "{{ route('cigarate.store_cigarate') }}";
        }
 
        if($('#action').val() == 'Edit')
        {
            action_url = "{{ route('cigarate.update_cigarate') }}";
        }
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if(data.success)
                {
                    ...
                    $('#sample_form')[0].reset();
                    $('#cigarate_table').DataTable().ajax.reload();
    ...
    $(document).on('click', '.edit', function(event){
        ...
        $.ajax({
            ...
            success:function(data)
            {
                ...
                $('#packet_price').val(data.result.packet_price);
                $('#number_in_pack').val(data.result.number_in_pack);
                $('#hidden_id').val(id);
                $('.modal-title').text('Edit Cigarate');
    ...
    $(document).on('click', '.detail', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#attributeDetails1').html('<p>'+'<strong> Half-Price:</strong>'+'<br>' + attribute.half_price + '</p>');
                $('#attributeDetails2').html('<p>'+'<strong> Packet-Price:</strong>'+'<br>' + attribute.packet_price + '</p>');
                $('#attributeDetails3').html('<p>'+'<strong> Number of items in one pack:</strong>'+'<br>' + attribute.number_in_pack + '</p>');
    ...
    $(document).on('click', '.delete', function(){
        cigarate_id = $(this).attr('id');
        $('.modal-title').text('Delete Cigarate');
        $('#confirmModal').modal('show');
    });
 
    $('#ok_button').click(function(){
        $.ajax({
            url:"/material/cigarate/delete/"+cigarate_id,
            beforeSend:function(){
