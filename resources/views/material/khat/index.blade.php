        ...
        <div class="breadcrumb-line breadcrumb-line-light header-elements-lg-inline">
            <div class="d-flex">
                <div class="breadcrumb">
                    ...
                    <span class="breadcrumb-item active">Khats</span>
                </div>
...
<div class="content">
         ...
         <div class="col-12 table-responsive">
            <div class="card">
                <div class="pl-3 card-header" style='background-color:whitesmoke' >
                    <button type="button" name="create_record" id="create_record" class="float-right btn btn-sm btn-primary mr-3"><i class="bi bi-plus-square"></i> Add Khat</button>
					<h3 class="mb-0;ml:5"><span style="font-family: serif;color:black;weight:bolder"><strong>Khats List</strong></span></h3>
                </div>
                ...
                <div class="card-body pt-1" >
                    ...
                    <div class='row table-responsive'>
                        <table style='width:100%'  class=" table table-bordered table-condensed mytable table-striped khat_table">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Name</th>
    ...
    <div class="modal fade" id="formModal" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
        ...
        <form method="post" id="sample_form" class="form-horizontal">
            <div class="modal-header" style="background-color: lightgray; text-align: center;">
                <h5 class="modal-title" id="ModalLabel" style="color: white;">Add New Khat</h5>
    ...
    <div class="modal fade" id="attributeModal" tabindex="-1" role="dialog" aria-labelledby="attributeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header" style="background-color: lightgray; text-align: center;">
                    <h5 class="modal-title" id="attributeModalLabel" style="color: white;">Khat Details</h5>
...
$(document).ready(function() {
    var table = $('.khat_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('khat.view_khat') }}",
    ...
    dataTableSearch = $('.khat_table').DataTable();  
    ...
    $('#sample_form').on('submit', function(event){
        ...
        if($('#action').val() == 'Add')
        {
            action_url = "{{ route('khat.store_khat') }}";
        }
 
        if($('#action').val() == 'Edit')
        {
            action_url = "{{ route('khat.update_khat') }}";
        }
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if(data.success)
                {
                    ...
                    $('#sample_form')[0].reset();
                    $('#khat_table').DataTable().ajax.reload();
    ...
    $(document).on('click', '.edit', function(event){
        ...
        $.ajax({
            ...
            success:function(data)
            {
                ...
                $('#buying_price').val(data.result.buying_price);
                $('#selling_price').val(data.result.selling_price);
                $('#hidden_id').val(id);
                $('.modal-title').text('Edit Khat');
    ...
    $(document).on('click', '.detail', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#attributeDetails1').html('<p>'+'<strong> Buying Price:</strong>'+'<br>' + attribute.buying_price + '</p>');
                $('#attributeDetails2').html('<p>'+'<strong> Selling Price:</strong>'+'<br>' + attribute.selling_price + '</p>');
    ...
    $(document).on('click', '.delete', function(){
        khat_id = $(this).attr('id');
        $('.modal-title').text('Delete Khat');
        $('#confirmModal').modal('show');
    });
 
    $('#ok_button').click(function(){
        $.ajax({
            url:"/material/khat/delete/"+khat_id,
            beforeSend:function(){
